...
const SERVERS: Record<string, string> = {
  nacd1: 'https://s10-nacd1.dentons.com/DentonsServices/DentonsPeopleSearch.asmx/SearchResultData',
  eucd1: 'https://s10-eucd1.dentons.com/DentonsServices/DentonsPeopleSearch.asmx/SearchResultData',
}
...
async function fetchPage(endpoint: string, page: number, size: number): Promise<{ totalResult: number; persons: PersonItem[] }> {
  ...
  return { totalResult: data.totalResult, persons: data.persons || [] }
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const nacd1Links = new Map<string, PersonItem>()
    const eucd1Links = new Map<string, PersonItem>()
    ...
    for (let page = 1; page <= totalPages; page++) {
      ...
      for (const p of nData.persons) {
        nacd1Links.set(normalizePath(p.link), p)
      }
      for (const p of eData.persons) {
        eucd1Links.set(normalizePath(p.link), p)
      }

      const onlyInN: PersonItem[] = []
      const onlyInE: PersonItem[] = []
      ...
      if (foundDiff === diff && (onlyInN.length + onlyInE.length) > 0) {
        return NextResponse.json({
          ...
          eucd1Total,
          difference: diff,
          onlyInNacd1: onlyInN.map(p => ({
            name: `${p.firstName} ${p.lastName || ''}`.trim(),
            jobTitle: p.jobTitle,
            link: p.link,
            office: p.officeDetails || '',
          })),
          onlyInEucd1: onlyInE.map(p => ({
            name: `${p.firstName} ${p.lastName || ''}`.trim(),
            jobTitle: p.jobTitle,
            link: p.link,
            office: p.officeDetails || '',
    ...
    console.error('People compare error:', error)
    return NextResponse.json(
