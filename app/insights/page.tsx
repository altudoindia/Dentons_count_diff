...
export default function InsightsPage() {
  ...
  const [state, setState] = useState<Record<Service, ServiceState>>({
    insights: { ...initialServiceState },
    people: { ...initialServiceState },
  ...
  const updateService = useCallback((svc: Service, patch: Partial<ServiceState>) => {
    setState(prev => ({ ...prev, [svc]: { ...prev[svc], ...patch } }))
  }, [])

  const fetchBatchCounts = useCallback(async (domain: string, side: 'left' | 'right', runId: number) => {
    setState(prev => ({
      ...prev,
      insights: { ...prev.insights, [side]: { loading: true, error: null, count: null } },
      people: { ...prev.people, [side]: { loading: true, error: null, count: null } },
      news: { ...prev.news, [side]: { loading: true, error: null, count: null } },
    }))

    try {
      const res = await fetch(`/api/batch-counts?domain=${encodeURIComponent(domain)}`, { cache: 'no-store' })
      ...
      const insightsData = json.insights ?? {}
      const peopleData = json.people ?? {}
      const newsData = json.news ?? {}
      const insightsCount = insightsData.count ?? null
      const peopleCount = peopleData.count ?? null
      const newsCount = newsData.count ?? null

      setState(prev => {
        if (compareRunIdRef.current !== runId) return prev
        return {
          ...prev,
          insights: { ...prev.insights, [side]: { count: insightsCount, loading: false, error: insightsData.error ?? null } },
          people: { ...prev.people, [side]: { count: peopleCount, loading: false, error: peopleData.error ?? null } },
          news: { ...prev.news, [side]: { count: newsCount, loading: false, error: newsData.error ?? null } },
        }
      })
    } catch (err) {
      const msg = (err as Error).message
      setState(prev => (compareRunIdRef.current !== runId ? prev : {
        ...prev,
        insights: { ...prev.insights, [side]: { count: null, loading: false, error: msg } },
        people: { ...prev.people, [side]: { count: null, loading: false, error: msg } },
        news: { ...prev.news, [side]: { count: null, loading: false, error: msg } },
      }))
    }
  }, [])
  ...
  const fetchEvents = useCallback(async (domain: string, type: string, setter: (d: EventsData) => void) => {
    ...
    try {
      const res = await fetch(`/api/events?domain=${domain}&type=${type}`)
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      const json = await res.json()
  ...
  const runCompare = useCallback(async (service: Service) => {
    updateService(service, { comparing: true, compareError: null, compare: null })
    try {
      ...
      const json = await res.json()
      if (json.error) throw new Error(json.message)
      updateService(service, { compare: json, comparing: false })
    } catch (err) {
      updateService(service, { compareError: (err as Error).message, comparing: false })
    }
  }, [leftDomain, rightDomain, updateService])
      ...
      <header className="bg-[#1a1a2e] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="py-2 text-xs text-gray-400">
            <span>Home</span><span className="mx-1">&gt;</span><span>Server Comparison</span>
          </nav>
