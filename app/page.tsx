...
function EmptyJobTitleBiosSection({
  leftDomain,
  rightDomain,
  leftLabel,
  rightLabel,
  ...
  const loadEmptyJobTitleBios = useCallback(() => {
    ...
    Promise.all([
      fetch(leftUrl, { cache: 'no-store', headers: { Pragma: 'no-cache', 'Cache-Control': 'no-cache' } }).then(r => r.json()),
      fetch(rightUrl, { cache: 'no-store', headers: { Pragma: 'no-cache', 'Cache-Control': 'no-cache' } }).then(r => r.json()),
    ]).then(([leftJson, rightJson]) => {
      setLeftData({
        items: leftJson.items ?? [],
        loading: false,
        error: leftJson.error ? (leftJson.message || String(leftJson.error)) : null,
      })
      setRightData({
        items: rightJson.items ?? [],
        loading: false,
        error: rightJson.error ? (rightJson.message || String(rightJson.error)) : null,
      })
    }).catch(err => {
      setLeftData(prev => ({ ...prev, loading: false, error: (err as Error).message }))
      setRightData(prev => ({ ...prev, loading: false, error: (err as Error).message }))
    })
  }, [leftDomain, rightDomain])
  ...
  return (
    <section className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <h3 className="text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wider">{leftLabel}</h3>
          {leftData.error && <p className="text-red-500 text-xs mb-2">{leftData.error}</p>}
          {leftData.items.length === 0 && !leftData.loading && !leftData.error && (
            <p className="text-gray-400 text-sm">Click &quot;Load bios&quot; to fetch items with empty job title.</p>
          )}
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {leftData.items.map((item, i) => (
          ...
          {leftData.items.length > 0 && (
            <p className="text-xs text-gray-500 mt-2">{leftData.items.length} bio(s) with empty job title</p>
          )}
        </div>
        <div>
          <h3 className="text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wider">{rightLabel}</h3>
          {rightData.error && <p className="text-red-500 text-xs mb-2">{rightData.error}</p>}
          {rightData.items.length === 0 && !rightData.loading && !rightData.error && (
            <p className="text-gray-400 text-sm">Click &quot;Load bios&quot; to fetch items with empty job title.</p>
          )}
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {rightData.items.map((item, i) => (
              <div key={i} className="border border-gray-100 rounded-lg p-3 bg-gray-50/50">
                <p className="font-medium text-sm text-gray-900">
                  {[item.firstName, item.lastName].filter(Boolean).join(' ').trim() || '—'}
                </p>
          ...
          {rightData.items.length > 0 && (
            <p className="text-xs text-gray-500 mt-2">{rightData.items.length} bio(s) with empty job title</p>
          )}
        </div>
      </div>
    </section>
